...
const objects = ref([]);
const targets = ref({ table: [], sphere: [], helix: [], grid: [] });
const table = [
...
const changeMenu = (key) => {
  transform(targets.value[key], 2000);
};

const init = () => {
  camera.value = new THREE.PerspectiveCamera(40, width.value / height.value, 1, 10000);
  ...
  for (let i = 0; i < table.length; i += 5) {
    ...
    card.className = 'card';
    card.style.backgroundColor = 'rgba(0,127,127,' + (Math.random() * 0.5 + 0.25) + ')';
  ...
  document.getElementById('card3d-container').appendChild(renderer.value.domElement);
...
onMounted(() => {
  width.value = document.getElementById('card3d-container').offsetWidth;
  height.value = document.getElementById('card3d-container').offsetHeight;
  init();
  animate();
